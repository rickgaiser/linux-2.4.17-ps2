/* $USAGI: pfkey_v2_msg_add.c,v 1.6 2002/03/05 07:50:37 mk Exp $ */
/*
...
#include "sadb_utils.h" /* esptoa authtoa */
			/* sockaddrtoa ultoa */
#define BUFSIZE 64
...
int sadb_msg_add_parse(struct sock *sk, struct sadb_msg *msg, struct sadb_msg **reply)
{
		 ...
		 ext_msgs[SADB_EXT_KEY_ENCRYPT]))
	{
		...
		if ((!sa->sadb_sa_auth && !sa->sadb_sa_encrypt) ||
		...
		if (sa->sadb_sa_encrypt == SADB_EALG_NULL && sa->sadb_sa_auth == SADB_AALG_NONE) {
			error = -EINVAL;
			goto err;
		}
		...
		if (ext_msgs[SADB_EXT_ADDRESS_PROXY]) {
			pxy = (struct sadb_address*)ext_msgs[SADB_EXT_ADDRESS_PROXY];
			error = sadb_address_to_sockaddr( pxy,
					(struct sockaddr*)&sa_entry->pxy);
			if (error) {
				PFKEY_DEBUG("pxy translation failed\n");
				goto err;
			}
		}
