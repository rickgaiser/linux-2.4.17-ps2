/* $USAGI: spd.c,v 1.3 2002/03/05 07:50:37 mk Exp $ */
/*
...
int spd_find_by_selector(struct selector *selector, struct ipsec_sp **policy)
{
	...
	list_for_each(pos, &spd_list){
		tmp_sp = list_entry(pos, struct ipsec_sp, entry);
		read_lock(&tmp_sp->lock);
		if (!compare_selector(selector, &tmp_sp->selector)) {
			SPD_DEBUG("found matched element\n");
			error = -EEXIST;
			*policy = tmp_sp;
			atomic_inc(&(*policy)->refcnt);
			read_unlock(&tmp_sp->lock);
			break;
		}
		read_unlock(&tmp_sp->lock);
	}
...
static int spd_get_info(char *buffer, char **start, off_t offset, int length)
{
	...
	int len = 0;
		...
		read_lock(&tmp_sp->lock);
                ...
                len += sprintf(buffer + len, "src:%s/%d\n", buf, tmp_sp->selector.prefixlen_s);
		...
		len += sprintf(buffer + len, "sport:%s\n", buf);
                ...
                len += sprintf(buffer + len, "dst:%s/%d\n", buf, tmp_sp->selector.prefixlen_d);
		sockporttoa((struct sockaddr *)&tmp_sp->selector.dst, buf, BUFSIZE);
		len += sprintf(buffer + len, "dport:%s\n", buf);
		len += sprintf(buffer + len, "proto:%u\n", tmp_sp->selector.proto);
		len += sprintf(buffer + len, "policy:%u\n", tmp_sp->policy_action);

		if (tmp_sp->auth_sa_idx) {
			len += sprintf(buffer + len, "sa(ah):%p\n", tmp_sp->auth_sa_idx->sa);
			sockaddrtoa((struct sockaddr*)&tmp_sp->auth_sa_idx->dst, buf, BUFSIZE);
			len += sprintf(buffer + len, "dst:%s/%d\n", buf, tmp_sp->auth_sa_idx->prefixlen_d);
			len += sprintf(buffer + len, "ipsec_proto:%u ",  tmp_sp->auth_sa_idx->ipsec_proto);
			len += sprintf(buffer + len, "spi:0x%x\n", htonl(tmp_sp->auth_sa_idx->spi));
		}

		if (tmp_sp->esp_sa_idx) {
			len += sprintf(buffer + len, "sa(esp):%p\n", tmp_sp->esp_sa_idx->sa);
			sockaddrtoa((struct sockaddr*)&tmp_sp->esp_sa_idx->dst, buf, BUFSIZE);
			len += sprintf(buffer + len, "dst:%s/%d\n", buf, tmp_sp->esp_sa_idx->prefixlen_d);
			len += sprintf(buffer + len, "ipsec_proto:%u ",  tmp_sp->esp_sa_idx->ipsec_proto);
			len += sprintf(buffer + len, "spi:0x%x\n", htonl(tmp_sp->esp_sa_idx->spi));
		}

		if (tmp_sp->comp_sa_idx) {
			len += sprintf(buffer + len, "sa(comp):%p\n", tmp_sp->comp_sa_idx->sa);
			sockaddrtoa((struct sockaddr*)&tmp_sp->comp_sa_idx->dst, buf, BUFSIZE);
			len += sprintf(buffer + len, "dst:%s/%d\n", buf, tmp_sp->comp_sa_idx->prefixlen_d);
			len += sprintf(buffer + len, "ipsec_proto:%u ",  tmp_sp->comp_sa_idx->ipsec_proto);
