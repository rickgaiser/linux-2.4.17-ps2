/* $USAGI: ndisc.c,v 1.79 2002/01/21 14:49:53 yoshfuji Exp $ */
...
union ndisc_options *ndisc_parse_options(u8 *opt, int opt_len, union ndisc_options *ndopts){
		   ...
		   int router, int solicited, int override, int inc_opt) 
{
	...
	if (ifp) {
		...
		if (ipv6_dev_get_saddr(dev, daddr, &saddr_tmp)) {
#ifdef CONFIG_IPV6_NDISC_DEBUG
			char abuf[64];
			in6_ntop(daddr, abuf);
...
static void ndisc_ns_rcv(struct sk_buff *skb)
{
	...
	if (saddr_type == IPV6_ADDR_ANY) {
		...
		if (!(daddr->s6_addr32[0] == __constant_htonl(0xff020000) &&
		      daddr->s6_addr32[1] == __constant_htonl(0x00000000) &&
		      daddr->s6_addr32[2] == __constant_htonl(0x00000001) &&
	...
	if (!ndisc_parse_options((u8*)(msg + 1), ndoptlen, &ndopts)){
		if (net_ratelimit())
...
bad:
	MIPV6_CALLFUNC(mipv6_check_dad, 0)(&msg->target);
	return;
}
...
static void ndisc_ra_rcv(struct sk_buff *skb)
{
	...
	if (!ndisc_parse_options((u8*)(ra_msg+1), ndoptlen, &ndopts)){
		if (net_ratelimit())
			ND_PRINTK2(KERN_WARNING "ICMP6 RA: invalid ND option, ignored\n");
		goto out;
	}
	...
	/*
 	 * To avoid adding new default routers if the old one is still in use
	*/
	mipv6_test=MIPV6_CALLFUNC(mipv6_ra_rcv, 1)(skb);
	if (rt == NULL && lifetime && mipv6_test) {
		ND_PRINTK3(KERN_DEBUG "ndisc_ra_rcv: adding default router\n");
