 ...
 *
 *	$Id: ipv6_tunnel.c,v 1.2.4.1 2002/05/28 14:42:10 nakamura Exp $
 *
...
static struct ipv6_tunnel_hook_ops *hooks[IPV6_TUNNEL_MAXHOOKS];
...
#ifdef TUNNEL_DEBUG
static void print_in6_addr(char *label, struct in6_addr *addr)
{
	printk("%s: %x:%x:%x:%x:%x:%x:%x:%x,\n", 
	       label, ntohs(addr->s6_addr16[0]), 
...
#ifdef TUNNEL_DEBUG_TRAFFIC
#include <linux/netfilter_ipv6.h>
#include <linux/netfilter.h>

static __inline__ int 
ipv6_tunnel_addr_type_valid(struct in6_addr *saddr, int addr_type);
			      ...
			      int (*okfn)(struct sk_buff *)) 
{
	...
	if (!ipv6_tunnel_addr_type_valid(&hdr->saddr, IPV6_ADDR_UNICAST) ||
	    !ipv6_tunnel_addr_type_valid(&hdr->daddr, IPV6_ADDR_UNICAST))
	    return NF_ACCEPT;

	printk(KERN_DEBUG "Packet arrived at pre routing hook...\n");
	print_in6_addr("from", &hdr->saddr);
	print_in6_addr("to", &hdr->daddr);
	if (dev)
		printk(KERN_DEBUG "on: %s\n", dev->name);
	
	return NF_ACCEPT;
}

unsigned int local_in_hook(unsigned int hooknum, 
			   struct sk_buff **skb,
			   const struct net_device *in,
			   const struct net_device *out,
			   int (*okfn)(struct sk_buff *)) 
{
	...
	printk(KERN_DEBUG "Packet arrived at local in hook...\n");
	print_in6_addr("from", &hdr->saddr);
	print_in6_addr("to", &hdr->daddr);
	if (dev)
		printk(KERN_DEBUG "on: %s\n", dev->name);
...
unsigned int forward_hook(unsigned int hooknum, 
			  struct sk_buff **skb,
			  const struct net_device *in,
			  const struct net_device *out,
			  int (*okfn)(struct sk_buff *)) 
{
	...
	printk(KERN_DEBUG "Packet arrived at forward hook...\n");
	print_in6_addr("from", &hdr->saddr);
	print_in6_addr("to", &hdr->daddr);
	if (dev)
		printk(KERN_DEBUG "on: %s\n", dev->name);
...
unsigned int local_out_hook(unsigned int hooknum, 
			    struct sk_buff **skb,
			    const struct net_device *in,
			    const struct net_device *out,
			    int (*okfn)(struct sk_buff *)) 
{
	...
	printk(KERN_DEBUG "Packet arrived at local out hook...\n");
	print_in6_addr("from", &hdr->saddr);
	print_in6_addr("to", &hdr->daddr);
	if (dev)
		printk(KERN_DEBUG "on: %s\n", dev->name);
			       ...
			       int (*okfn)(struct sk_buff *)) 
{
	...
	printk(KERN_DEBUG "Packet arrived at post routing hook...\n");
	print_in6_addr("from", &hdr->saddr);
	print_in6_addr("to", &hdr->daddr);
	if (dev)
		printk(KERN_DEBUG "on: %s\n", dev->name);
...
static struct nf_hook_ops ipv6_tunnel_ops[] = { { { NULL, NULL }, 
						  pre_routing_hook, 
						  PF_INET6, 
						  NF_IP6_PRE_ROUTING, 
						  NF_IP6_PRI_FIRST },
...
/**
 * ipv6_ipv6_kernel_tunnel_put - cache discarded kernel tunnel
 *   @t: kernel tunnel
 *
 * Description:
 *   ipv6_ipv6_kernel_tunnel_put() caches a discarded kernel tunnel so the
 *   structure can be reused later.
 * 
 * Caveat:
 *   t->parms.flags.kernel_dev is assumed to be raised.
 *
 * Note:
 *   See the comments on ipv6_ipv6_kernel_tunnel_add() for more information.
 **/

static __inline__ void ipv6_ipv6_kernel_tunnel_put(struct ipv6_tunnel *t)
{
#ifdef TUNNEL_DEBUG
	printk(KERN_DEBUG "ipv6_ipv6_kernel_tunnel_put\n");
#endif /* TUNNEL_DEBUG */
...
/**
 * ipv6_ipv6_kernel_tunnel_get - get first free kernel device
 *
 * Description:
 *   ipv6_ipv6_kernel_tunnel_get() fetches a kernel tunnel previously
 *   created by ipv6_ipv6_kernel_tunnel_add().
 * 
...
static __inline__ struct ipv6_tunnel *ipv6_ipv6_kernel_tunnel_get(void)
{
	struct ipv6_tunnel *t;
#ifdef TUNNEL_DEBUG
	printk(KERN_DEBUG "ipv6_ipv6_kernel_tunnel_get\n");
#endif /* TUNNEL_DEBUG */
	write_lock(&ipv6_ipv6_kernel_lock);
	if ((t = tunnels_kernel[0]) != NULL) {
		tunnels_kernel[0] = t->next;
	}
	write_unlock(&ipv6_ipv6_kernel_lock);
	return t;
}
...
#endif /* TUNNEL_DEBUG */
	ipv6_ipv6_kernel_tunnel_put(t);
}


/**
 * ipv6_ipv6_kernel_tunnel_unlink - remove kernel tunnel from cache
 *   @t: kernel tunnel
 *
 * Note:
 *   See the comments on ipv6_ipv6_kernel_tunnel_add() for more information.
...
static void ipv6_ipv6_kernel_tunnel_unlink(struct ipv6_tunnel *t)
{
...
#endif /* TUNNEL_DEBUG */
	write_lock_bh(&ipv6_ipv6_kernel_lock);
	for (tp = &tunnels_kernel[0]; *tp; tp = &(*tp)->next) {
		if (t == *tp) {
			*tp = t->next;
			break;
		}
...
/**
 * ipv6_ipv6_kernel_tunnel_unlink_head - remove first kernel tunnel from cache
 *
 * Return: first free kernel tunnel
 *
 * Note:
...
static __inline__ struct ipv6_tunnel 
*ipv6_ipv6_kernel_tunnel_unlink_head(void)
{
#ifdef TUNNEL_DEBUG
	printk(KERN_DEBUG "ipv6_ipv6_kernel_tunnel_unlink_head\n");
#endif /* TUNNEL_DEBUG */
	return ipv6_ipv6_kernel_tunnel_get();
}
...
#endif /* TUNNEL_DEBUG */
	return (ipv6_addr_cmp(saddr, daddr) &&
	   ...
	   struct sk_buff *skb, __u32 flags)
{
	...
	if (hooknum < IPV6_TUNNEL_MAXHOOKS) {
		...
		for (h = hooks[hooknum]; h != NULL; h = h->next)
			if (h->hook && 
			    (accept = h->hook(t,skb, flags)) != IPV6_TUNNEL_ACCEPT)
				break;
 ...
 *
 * Description:
 *   Avoid tunneling loops by checking that tunnel endpoints don't match.
 *
 * Return: 
 *   1 if no conflict,
 *   0 else
...
#endif /* TUNNEL_DEBUG */
	return ((!ipv6_addr_cmp(&t->parms.saddr, &hdr->saddr) &&
		 !ipv6_addr_cmp(&t->parms.daddr, &hdr->daddr)) ||
		!ipv6_addr_cmp(&t->parms.daddr, &hdr->saddr));
}
 ...
 * Return: 
 *   0 on success,
 *   %-EAGAIN if the ipv6 socket is already in use by some other processor.
 **/
...
static int ipv6_ipv6_tunnel_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	ip6_build_xmit(sk, ipv6_ipv6_getfrag, (void *)skb->nh.raw, 
		       &fl,skb->len, opt, t->parms.hop_limit, 0, 0);
	ipv6_xmit_unlock();
...
static void ipv6_ipv6_tunnel_dev_config(struct net_device *dev, int active)
{
	struct ipv6_tunnel *t = (struct ipv6_tunnel *)dev->priv;
	...
	/* Locate actual device the modified packets should be sent to 
	   after the tunnel has added its headers to them. */
	if (active) {
		dst = ip6_route_output(ipv6_socket->sk, fl);
		
		if (!dst->error)
			tdev = dst->dev;	
		dst_release(dst);	
		
		if (!tdev && t->parms.link) {
			read_lock_bh(&dev_base_lock);
			tdev = __dev_get_by_index(t->parms.link);
			read_unlock_bh(&dev_base_lock);
		}
		if (tdev) {
			dev->hard_header_len = tdev->hard_header_len + sizeof(struct ipv6hdr);
			dev->mtu = tdev->mtu - sizeof(struct ipv6hdr);
			
			if (dev->mtu < IPV6_MIN_MTU)
				dev->mtu = IPV6_MIN_MTU;
		}
	}
	dev->iflink = t->parms.link;
	return;
}
				   ...
				   int active)
{
	...
	ipv6_ipv6_tunnel_dev_config(dev, active);
 ...
 *   %-EINVAL if p->flags.kernel_dev isn't raised,
...
int ipv6_ipv6_kernel_tunnel_add(struct ipv6_tunnel_parm *p)
{
	...
	if ((t = ipv6_ipv6_kernel_tunnel_get()) == NULL) {
		return -ENODEV;
	}
	ipv6_ipv6_tunnel_change(t, p, 1);

	if ((err = ipv6_tunnel_addr_add(t->dev, &p->saddr)) != 0) {
		ipv6_ipv6_kernel_tunnel_put(t);
		return err;
	}
...
int ipv6_ipv6_kernel_tunnel_del(struct ipv6_tunnel *t)
{
	...
	if (t != NULL) {
		...
		ipv6_tunnel_addr_del(t->dev, &t->parms.saddr);
...
ipv6_ipv6_tunnel_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCDELTUNNEL:
		...
		if (dev == &ipv6_ipv6_fb_tunnel_dev) {
			...
			if (p.flags & IPV6_T_F_KERNEL_DEV) {
				t = ipv6_ipv6_kernel_tunnel_unlink_head();
				if(t == NULL) {
					err = -ENODEV;
					goto done;
				}
			...
			if (t->parms.flags & IPV6_T_F_KERNEL_DEV) {
				ipv6_ipv6_kernel_tunnel_unlink(t);
			}
		}
...
#endif /* TUNNEL_DEBUG */
	if (new_mtu < IPV6_MIN_MTU || new_mtu > 0xFFF8 - sizeof(struct ipv6hdr)) {
		return -EINVAL;
}
	dev->mtu = new_mtu;
	return 0;
...
{	
	...
	ipv6_ipv6_tunnel_dev_config(dev, 
				    !(t->parms.flags & IPV6_T_F_KERNEL_DEV));
...
void ipv6_ipv6_tunnel_register_hook(struct ipv6_tunnel_hook_ops *reg) 
{
	...
	if (reg->hooknum < IPV6_TUNNEL_MAXHOOKS) {
		...
		reg->next = hooks[reg->hooknum];
		hooks[reg->hooknum] = reg;
...
void ipv6_ipv6_tunnel_unregister_hook(struct ipv6_tunnel_hook_ops *reg)
{
	...
	if (reg->hooknum < IPV6_TUNNEL_MAXHOOKS) {
		struct ipv6_tunnel_hook_ops *h;
		write_lock_bh(&ipv6_ipv6_hook_lock);		
		for (h = hooks[reg->hooknum]; h; h = h->next) {
			if (reg == h) {
				h = reg->next;
				break;
			}
...
int __init ipv6_ipv6_tunnel_init(void)
{
	...
	if ((err = sock_create(PF_INET6, SOCK_RAW, IPPROTO_IPV6, &ipv6_socket)) < 0) {
		printk(KERN_ERR
...
#ifdef TUNNEL_DEBUG_TRAFFIC
	printk(KERN_DEBUG "Registering net filter hooks.\n");
	nf_register_hook(&ipv6_tunnel_ops[0]);
	nf_register_hook(&ipv6_tunnel_ops[1]);
	nf_register_hook(&ipv6_tunnel_ops[2]);
	nf_register_hook(&ipv6_tunnel_ops[3]);
	nf_register_hook(&ipv6_tunnel_ops[4]);
#endif
#endif
	return 0;
}
...
#ifdef TUNNEL_DEBUG_TRAFFIC
	printk(KERN_DEBUG "Unregistering net filter hooks.\n");
	nf_unregister_hook(&ipv6_tunnel_ops[4]);
	nf_unregister_hook(&ipv6_tunnel_ops[3]);
	nf_unregister_hook(&ipv6_tunnel_ops[2]);
	nf_unregister_hook(&ipv6_tunnel_ops[1]);
	nf_unregister_hook(&ipv6_tunnel_ops[0]);
#endif
#endif
