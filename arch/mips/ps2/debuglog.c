 ...
 *
 * $Id: debuglog.c,v 1.1.2.3 2002/04/18 10:21:07 takemura Exp $
 */
...
static spinlock_t debuglog_lock = SPIN_LOCK_UNLOCKED;
...
getdefaultlog(void)
{
	...
	spin_lock_irqsave(&debuglog_lock, flags);
	if (!debuglog_inited) {
		debuglog_init(&default_log, NULL, debuglog_size);
		debuglog_inited = 1;
	}
	spin_unlock_irqrestore(&debuglog_lock, flags);
...
module_init(ps2bdglog_start);
module_exit(ps2bdglog_exit);

MODULE_AUTHOR("Sony Computer Entertainment Inc.");
MODULE_DESCRIPTION("debugging log module");
MODULE_LICENSE("GPL");
