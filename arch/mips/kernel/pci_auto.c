...
void __init 
pciauto_setup_bars_cardbus_bridge(struct pci_channel *hose,
		   int top_bus,
		   int current_bus,
		   int pci_devfn)
{
                             ...
                             int sub_bus)
{
       ...
       /* Align memory and I/O to 4KB and 4 byte boundaries. */
       /* Reserve 16KB I/O space */
       pciauto_lower_memspc = (pciauto_lower_memspc + (0x1000 - 1))
               & ~(0x1000 - 1);
       pciauto_lower_iospc = (pciauto_lower_iospc + (0x4000 - 1))
               & ~(0x4 - 1);
...
pciauto_bus_scan(struct pci_channel *hose, int top_bus, int current_bus)
{
	...
	if ((current_bus == top_bus) && (hose->first_devfn))
		devfn_start = hose->first_devfn;
	if ((current_bus == top_bus) && (hose->last_devfn))
	...
	for (pci_devfn=devfn_start; pci_devfn<devfn_stop; pci_devfn++) {
		...
		if ((pci_class >> 16) == PCI_CLASS_BRIDGE_PCI) {
			...
			continue;
                        ...
                        pciauto_setup_bars_cardbus_bridge(hose, top_bus, current_bus, pci_devfn);
