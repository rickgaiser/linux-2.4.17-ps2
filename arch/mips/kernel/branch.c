...
int __compute_return_epc(struct pt_regs *regs)
{
	...
	case spec_op:
		...
		case jalr_op:
			...
			/* Fall through */
		case jr_op:
			regs->cp0_epc = get_gpreg(regs, insn.r_format.rs);
			break;
		}
		break;
	...
	case bcond_op:
		switch (insn.i_format.rt) {
	 	case bltz_op:
		case bltzl_op:
			if ((long)get_gpreg(regs, insn.i_format.rs) < 0)
	 ...
	 */
	case jal_op:
		set_gpreg(regs, 31, regs->cp0_epc + 8);
	case j_op:
		epc += 4;
		epc >>= 28;
		epc <<= 28;
	...
	case beq_op:
	case beql_op:
		if (get_gpreg(regs, insn.i_format.rs) ==
		    get_gpreg(regs, insn.i_format.rt))
			epc = epc + 4 + (insn.i_format.simmediate << 2);
		else
			epc += 8;
		regs->cp0_epc = epc;
