 ...
 * This code is GPL
 *
 * $Id: cfi_cmdset_0002.c,v 1.58 2002/09/13 14:37:35 dwmw2 Exp $
 *
 */
...
#ifdef CONFIG_MTD_SNSC_MPU110
#include <asm/arch/snsc_mpu110.h>
#endif
#ifdef CONFIG_MPU110_LED_API
#include <asm/arch/misc.h>
#endif

#define CONFIG_MTD_SNSC_NOR_FIX
...
static int do_write_oneword(struct map_info *map, struct flchip *chip, unsigned long adr, __u32 datum, int fast)
{
 ...
 retry:
	...
	dq6 = CMD(1<<6);
	dq5 = CMD(1<<5);
#ifdef CONFIG_MTD_SNSC_NOR_FIX
	timeo = jiffies + (HZ/ 20); /* setting timeout to 50ms for now */
#else
	...
	if ((status & dq6) != (oldstatus & dq6)) {
		...
		/* still toggling, check if dq5 raised or jiffies timeout */
		...
		if ((status & dq5) == dq5) {
			...
			if ((status & dq6) != (oldstatus & dq6)) {
				/* still toggling, the operation is timedout
				 * or write error occured(attempt to program
				 * a bit which is 0 to 1).
				 * reset the chip and return to read-mode. */

				cfi_write(map, CMD(0xF0), chip->start);
				printk( KERN_WARNING "Internal flash device timeout occured or write operation was performed while flash was programming, or write operation was performed on not erased region.\n" );
				ret = -EIO;
				goto write_oneword_exit;
			}

			/* toggle stopped, dq5 is just bit5 of written datum
			 * (write success)                                 */
	...
	if ((status = cfi_read(map, adr)) != datum) {
		printk(KERN_WARNING "do_write_oneword: write failed. "
		       "written datum(%x)/original datum(%x)\n",
		       status, datum);
#ifdef CONFIG_MTD_SNSC_MPU110
		outl(inl(LED_BASE) & ~(1 << LED_BIT), LED_BASE);
		udelay(20);
		outl(inl(LED_BASE) | (1 << LED_BIT), LED_BASE);
#endif
	}
