 ...
 *
 * $Id: nand.c,v 1.32 2002/09/13 14:27:37 dwmw2 Exp $
 *
...
#if DEBUG_NAND_IO & 1
unsigned char __inline__  READB(unsigned int x) { 
    unsigned char tmp;
    tmp = readb(x);
    printk("readb(0x%x) = %x\n", (unsigned int)(x), tmp);
    return tmp; 
...
#  define WRITEB(x,y) do { printk("%s():%d:writeb(0x%x, 0x%x)\n", __FUNCTION__ , __LINE__ , (unsigned int)(x), (unsigned int)(y)); writeb(x,y); } while (0)
#else
...
static void nand_command (struct mtd_info *mtd, unsigned command, int column, int page_addr)
{
	...
	/* Don't issue command, while NAND chip is busy */
	/* Except RESET and READ STATUS command */
	if ((command != NAND_CMD_RESET) &&
	    (command != NAND_CMD_STATUS)) {
		if (this->dev_ready) {
			while (this->dev_ready() == 0)	/* Wait until ready */
				;
		} else {
			udelay (this->chip_delay);	/* Wait chip_delay us */
		}
	}
...
#ifndef CONFIG_MTD_NAND_FORCE_ERASE
			instr->state = MTD_ERASE_FAILED;
			goto erase_exit;
#endif
		}
#else
		{ /* print bad block flag for debug */
			unsigned char tmp;
			tmp = READB(this->IO_ADDR_R);
			if (tmp != 0xff) {
				printk(KERN_WARNING "nand_erase: attempt to erase a bad block at page 0x%08x, read=%02x\n", page, tmp);
#ifndef CONFIG_MTD_NAND_FORCE_ERASE
				instr->state = MTD_ERASE_FAILED;
				goto erase_exit;
#endif
...
#if 1
			/* write bad block information when erase failed. */
			{
				int ret, retlen;
				char buf = 0;

				spin_unlock_bh (&this->chip_lock);
				ret = nand_write_oob(mtd, (page << this->page_shift) + NAND_BADBLOCK_POS, 1, &retlen, &buf);
				if (ret < 0 || retlen != 1) {
					DEBUG (MTD_DEBUG_LEVEL0, "nand_erase: " "Failed to update oob badblock info.\n");
				}
				spin_lock_bh (&this->chip_lock);
			}
#endif
