...
//	This file contains implementations of the OS functions for Linux in
//	user-level code.
...
{
	*p_file_handle = creat(p_filename, S_IWRITE | S_IREAD);
	if (-1 == *p_file_handle)
...
{
	*p_file_handle = open(p_filename, O_RDONLY);
	if (-1 == *p_file_handle)
		return FS460_ERR_FILE_ERROR;
...
{
	if (-1 == read(file_handle, p_data, length))
		return FS460_ERR_FILE_ERROR;
...
{
	if (-1 == write(file_handle, p_data, length))
		return FS460_ERR_FILE_ERROR;
...
int OS_file_seek(unsigned long file_handle, long *p_new_offset, unsigned long offset, int origin)
{
	...
	new_offset = lseek(file_handle, offset, origin);
	if (-1 == new_offset)
		return FS460_ERR_FILE_ERROR;
