...
//	This file implements the Direct Memory abstraction layer for Windows NT
//	kernel-mode.
...
void DM_cleanup(void)
{
	...
	for (i= 0; i < CACHE_SIZE; i++)
	{
		...
		{
			MmUnmapIoSpace(cache[i].p_virt_address, 0x1000);
			cache[i].p_virt_address = 0;
...
static char *virt_address(unsigned long phys_address)
{
	LARGE_INTEGER li_address;
	void *p_virt_address;
	int i;
	...
	// map the address
	li_address.QuadPart = phys_address;
	p_virt_address = MmMapIoSpace(li_address, 0x1000, FALSE);

	// free the next cache location
	if (cache[cache_next].p_virt_address)
		MmUnmapIoSpace(cache[cache_next].p_virt_address, 0x1000);
...
{
	*p_data = READ_PORT_UCHAR((PUCHAR)port);
...
{
	*p_data = READ_PORT_USHORT((PUSHORT)port);
...
{
	*p_data = READ_PORT_ULONG((PULONG)port);
