/* $USAGI: sadb.h,v 1.27 2002/03/05 07:52:16 mk Exp $ */
/*
...
/* In theory, it would be nice to split the sa's in in and out lists.
...
struct ipsec_sa {
	...
	/* the sadb_address_proto field, normally zero,
	   MUST be filled in with the transport protocol's number */
	__u8 proto;
	...
	/* internal use, becase uint64 and jiffies are different unit */
	...
	/* tunnel_mode is a boolean flag if this is a tunnel mode sa */
};
...
struct sa_index{
	...
	/* sa doesn't always exist, if sa equals null it describe
	   suitable sa doesn't exist in sa list(it consists of struct ipsec_sa_t). */
        struct ipsec_sa *sa;
...
/* These functions expect the parameter sa is allocated. */
struct ipsec_sa* sadb_find_by_sa_index(struct sa_index *sa_index);
int sadb_find_by_address_proto_spi(struct sockaddr *src, __u8 prefixlen_s, struct sockaddr *dst, __u8 prefixlen_d,
				struct sockaddr *pxy, __u8 prefixlen_p,  __u8 ipsec_proto, __u32 spi, struct ipsec_sa **sa);

/* The struct sa_index is glue between SA and Policy */
