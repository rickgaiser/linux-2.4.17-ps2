...
asmlinkage long sys_shmget (key_t key, size_t size, int shmflg)
{
	...
	if (key == IPC_PRIVATE) {
		...
		if (shp->shm_segsz < size)
			...
			if (!(err = security_shm_associate(shp, shmid, shmflg)))
				err = shmid;
		}
		shm_unlock(id);
	}
...
asmlinkage long sys_shmctl (int shmid, int cmd, struct shmid_ds *buf)
{
...
#ifdef CONFIG_PS2_COMPAT_KERNEL22
	version = IPC_OLD;
#else
	...
	case IPC_INFO:
	{
		...
		if ((err = security_ipc_getinfo(shmid, cmd)))
			return err;
	...
	case IPC_STAT:
	{
		...
		if ((err = security_shm_shmctl(shp, shmid, cmd)))
			goto out_unlock;
...
asmlinkage long sys_shmat (int shmid, char *shmaddr, int shmflg, ulong *raddr)
{
	...
	if ((err = security_shm_shmat(shp, shmid, shmaddr, shmflg))) {
		shm_unlock(shmid);
		return err;
	}
